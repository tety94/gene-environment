...
from db import save_gene_result, gene_already_done, get_conn
...
def process_single_gene(gene_col, gene_original, Ecols):
    ...
    formula = build_formula(ONSET_COL, gene_col, Ecols, [], matched_obs)
    ...
    if obs_coef is None or abs(obs_coef) < MIN_OBS_COEF:
        save_gene_result(conn, gene_original, int(matched_obs[gene_col].sum()),
                         int((matched_obs[gene_col]==0).sum()), obs_coef, None, None, 1)
        conn.close()
